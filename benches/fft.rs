...
#[cfg(feature = "rt")]
use jlrs::{
    data::{layout::complex::Complex, managed::array::TypedRankedArray},
...
fn bench_forward_fft64(frame: &LocalGcFrame<0>, c: &mut Criterion) {
    ...
    for sz in [
        ...
        frame.local_scope::<_, 3>(|mut frame| unsafe {
            let mut planner = FftPlanner::<Complex<f64>>::new()
                .into_typed_value()
                .root(&mut frame);
...
fn bench_forward_fft32(frame: &LocalGcFrame<0>, c: &mut Criterion) {
    ...
    for sz in [
        ...
        frame.local_scope::<_, 3>(|mut frame| unsafe {
            let mut planner = FftPlanner::<Complex<f32>>::new()
                .into_typed_value()
                .root(&mut frame);
            let instance = planner
                .track_exclusive()
